...
- https://github.com/Open-Catalyst-Project/ocp/blob/main/ocpmodels/modules/exponential_moving_average.py
  (MIT license)
"""
...
class ExponentialMovingAverage:
    ...
    def __init__(
        ...
        self.collected_params: Iterable[torch.Tensor] = []
...
class EMAContextManager:
    """Context manager for ExponentialMovingAverage.

    Args:
        ema: The ExponentialMovingAverage object.
    """

    def __init__(self, ema: Optional[ExponentialMovingAverage]):
        self.ema = ema

    def __enter__(self):
        """Store the current parameters for restoring later."""
        if self.ema is not None:
            # Save current parameters to collected_params
            self.ema.store()
            # Copy shadow params to parameters
            self.ema.copy_to()

    def __exit__(self, exc_type, exc_val, exc_tb):
        """Restore the parameters stored with the `store` method."""
        if self.ema is not None:
            # Copy collected_params to parameters
            self.ema.restore()
