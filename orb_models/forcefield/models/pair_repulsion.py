...
class ZBLBasis(torch.nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "d",
            torch.tensor([3.2, 0.9423, 0.4028, 0.2016], dtype=torch.get_default_dtype()).unsqueeze(
                1
            ),
            persistent=False,
        )
    ...
    def forward(self, batch: AtomGraphs) -> dict:
        ...
        envelope, denvelope_dr = self._polynomial_cutoff_with_derivative(x, r_max, self.p)
