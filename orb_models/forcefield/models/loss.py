...
from orb_models.common.models.graph_regressor import bucketed_mean_error, mean_error
...
def force_metrics(
    ...
    metrics = {
        ...
        f"{name}_cosine_sim": torch.cosine_similarity(raw_pred, raw_target, dim=-1).mean(),
...
def _conditional_huber_force_loss(
    ...
    c2 = (torch.norm(target_forces, dim=-1) >= 100) & (torch.norm(target_forces, dim=-1) < 200)
    c3 = (torch.norm(target_forces, dim=-1) >= 200) & (torch.norm(target_forces, dim=-1) < 300)
    c4 = ~(c1 | c2 | c3)
