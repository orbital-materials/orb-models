...
def maybe_remove_net_force_and_torque(
    ...
    if remove_mean:
        system_means = segment_ops.aggregate_nodes(force_pred, batch.n_node, reduction="mean")
        node_broadcasted_means = torch.repeat_interleave(system_means, batch.n_node, dim=0)
...
def _remove_net_torque(
    ...
    # Compute M = S - sI
    I = (  # noqa: E741
        torch.eye(3, device=positions.device, dtype=positions.dtype).unsqueeze(0).expand(B, -1, -1)
    )  # Shape: (B, 3, 3)
