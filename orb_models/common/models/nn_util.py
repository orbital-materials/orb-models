...
class ChargeSpinEmbedding(nn.Module):
    ...
    def __init__(
        ...
        if self.embedding_type == "sin_emb" or self.embedding_type == "pos_emb":
            self.W = nn.Parameter(torch.randn(dim // 2) * scale, requires_grad=requires_grad)
...
class ChargeSpinConditioner(nn.Module):
    ...
    def forward(self, batch) -> tuple[torch.Tensor | None, torch.Tensor | None]:
...
def build_mlp(
    ...
    if (
        ...
        mlp = CheckpointedSequential(n_layers=nlayers, reentrant=checkpoint == "reentrant")
    else:
...
class CheckpointedSequential(nn.Sequential):
    ...
    def forward(self, input):
        ...
        if self.training:
            return checkpoint_sequential(self, self.n_layers, input, use_reentrant=self.reentrant)
        else:
...
class ScalarNormalizer(torch.nn.Module):
    ...
    def __init__(
        self,
        init_mean: torch.Tensor | float | None = None,
        init_std: torch.Tensor | float | None = None,
        init_num_batches: int | None = 1000,
        online: bool = True,
    ) -> None:
    ...
    def forward(self, x: torch.Tensor, online: bool | None = None) -> torch.Tensor:
