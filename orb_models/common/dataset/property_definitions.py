...
def get_property_from_row(
    ...
    valid_node_shape = len(property_tensor.shape) == 2 and property_tensor.shape[0] == row.natoms
    if not (valid_graph_shape or valid_node_shape):
        raise ValueError(
            f"Property {name} has invalid shape {property_tensor.shape} for row {row.id}"
        )
    assert property_tensor.dtype != torch.float32, (
...
class RowToPropertyFn(Protocol):
    """Protocol for row_to_property_fn."""

    def __call__(self, row: ase.db.row.AtomsRow, dataset: str) -> torch.Tensor: ...
...
class PropertyDefinition:
    ...
    name: str
    dim: int
    domain: Literal["real", "binary", "categorical", "string"]
    row_to_property_fn: RowToPropertyFn
    ...
    def fullname(self) -> str:
        ...
        return f"{self.name}-{self.level_of_theory}" if self.level_of_theory else self.name
...
def energy_row_fn(row: ase.db.row.AtomsRow, dataset: str):
    ...
    energy = get_property_from_row(extractor.name, row, extractor.mult, extractor.bias)  # type: ignore
    return energy
...
energy_d3_zero = PropertyDefinition(
    ...
    dim=1,
    domain="real",
    row_to_property_fn=lambda row, dataset: get_property_from_row("data.d3.energy", row),
...
forces_d3_zero = PropertyDefinition(
    ...
    dim=3,
    domain="real",
    row_to_property_fn=lambda row, dataset: get_property_from_row("data.d3.forces", row),
    level_of_theory="d3-zero",
)
...
stress_d3_zero = PropertyDefinition(
    ...
    dim=6,
    domain="real",
    row_to_property_fn=lambda row, dataset: get_property_from_row("data.d3.stress", row),
    level_of_theory="d3-zero",
)
...
class PropertyConfig:
    """Defines which properties should be extracted and stored on the AtomBatch.
    ...
    def extract(
        self, row: ase.db.row.AtomsRow, dataset_name: str, suffix: str | None = None
    ) -> dict:
...
def instantiate_property_config(
    config: MutableMapping[Any, Any] | None = None,
) -> PropertyConfig:
    """Get PropertyConfig object from Dict config."""
    if config is None:
        return PropertyConfig()
    assert all(key in ["node", "edge", "graph"] for key in config), (
